...
class Myridescancelled extends StatefulWidget {
  const Myridescancelled({super.key});

  @override
  State<Myridescancelled> createState() => _MyridescancelledState();
}

class _MyridescancelledState extends State<Myridescancelled> {
  late Future<List<RideModel>> _futureRides;

  @override
  void initState() {
    super.initState();
    _futureRides = DioHttp().getMyRides(context, 'cancelled');
  }
  ...
  Widget _buildReasonSection() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('REASON', style: GoogleFonts.nunito(fontSize: 11, fontWeight: FontWeight.w700, color: Color(0xff5A5A5A))),
          SizedBox(height: 4),
          Text('Driver is not responding', style: GoogleFonts.nunito(fontSize: 13, color: Color(0xffE02D3C), fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
