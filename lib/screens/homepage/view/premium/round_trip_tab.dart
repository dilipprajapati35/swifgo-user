...
class _RoundTripTabState extends State<RoundTripTab> {
  ...
  Future<void> _handleSubscribe() async {
    ...
    try {
      // 1. Navigate to location picker for round trip
      final locationResult = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (ctx) => SubscriptionLocationPickerPage(tripType: TripType.roundTrip),
        ),
      );
      if (locationResult == null || locationResult['pickupLatLng'] == null || locationResult['destinationLatLng'] == null || locationResult['returnPickupLatLng'] == null || locationResult['returnDestinationLatLng'] == null || locationResult['subscriptionStartDate'] == null) {
        setState(() => _isSubscribing = false);
        return;
      }
      final pickupLatLng = locationResult['pickupLatLng'];
      final destinationLatLng = locationResult['destinationLatLng'];
      final returnPickupLatLng = locationResult['returnPickupLatLng'];
      final returnDestinationLatLng = locationResult['returnDestinationLatLng'];
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (ctx) => SubscriptionTripSelectionScreen(
            ...
            returnDate: subscriptionStartDate.split('T')[0],
            returnTimePeriod: DateTime.parse(subscriptionStartDate).hour < 12 ? 'AM' : 'PM',
